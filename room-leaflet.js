/*! RoomLeaflet v1.0.0 | (c) GreenerSoft | https://roomjs.fr | MIT License */
...
async function MapContainer({id, className, provider, mount, unmount}) {
	...
	try {
		!L && (L = await import("Leaflet"));
		return div({id, class: className, onMount, onUnmount});
	} catch (e) {
		return p("Echec de chargement du module Leaflet.");
	}
}

export {EUCenter, FranceCenter, USACenter, OpenStreetMapProvider, L, createTileLayer, addTileLayer, MapContainer}
